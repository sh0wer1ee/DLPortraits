...
var localizationData = {
  ...
  "filelist-load-error-log": {
    ...
    en_us:
      "Error loading filelist.json, please check the console or refresh the page.",
...
function changeLang() {
  ...
  document.getElementById("title").innerText =
    localizationData["title"][language];
  //document.getElementById("label-chara-select").innerText = localizationData['chara-select'][language];
  document.getElementById("load-btn").innerText =
    localizationData["load-btn"][language];
  document.getElementById("label-face-select").innerText =
    localizationData["face-select"][language];
  document.getElementById("label-mouth-select").innerText =
    localizationData["mouth-select"][language];
  document.getElementById("border-btn").innerText =
    localizationData["border-btn"][language];
  document.getElementById("invert-btn").innerText =
    localizationData["invert-btn"][language];
  document.getElementById("reset-btn").innerText =
    localizationData["reset-btn"][language];
  document.getElementById("download-btn").innerText =
    localizationData["download-btn"][language];
  ...
  document.getElementById("close-modal").innerText =
    localizationData["close-modal"][language];
  document.getElementById("emotion-modal").innerText =
    localizationData["emotion-modal"][language];
  document.getElementById("emotionModalLabel").innerText =
    localizationData["emotionModalLabel"][language];
  document.getElementById("menu-toggle").innerText =
    localizationData["menu-toggle"][language];
  document.getElementById("sidebar-heading").innerText =
    localizationData["sidebar-heading"][language];
}
  ...
  fetch(`${portraitPath}localizedDirData.json`)
    ...
    .then((json) => {
      ...
      for (var key in json["fileList"]) {
        ...
        option.setAttribute(
          "data-image",
          portraitPath + "thumbnails/" + key + ".png"
        );
      ...
      p.forEach(function (value) {
        ...
        index.id = value;
        index.style =
          "font-size: 10px; text-decoration: underline; display:block; color: rgb(64, 112, 255); cursor: pointer;";
    ...
    .catch(function (error) {
      ...
      document.getElementById("log-text").innerText =
        localizationData["filelist-load-error-log"][language];
    });
}
  ...
  fetch("https://api.github.com/repos/sh0wer1ee/DLPortraits/branches/master")
    ...
    .then((json) => {
      ...
      document.getElementById(
        "update-time"
      ).innerText = `Last update: ${datestring}`;
    })
    .catch(function (error) {
      console.log(error);
...
document.getElementById("reset-btn").addEventListener("click", function () {
  if (loaded) {
    ...
    if (["110040_01", "110399_01", "110400_01"].includes(imgID)) {
      face2ID = "-1";
      mouth2ID = "-1";
      face2Img.src = "";
      mouth2Img.src = "";
  ...
  if (loaded) {
    if (!["110040_01", "110399_01", "110400_01"].includes(imgID)) {
      ...
      downloadCanvasAsImage(
        `${imgID}_${faceID}_${mouthID}_${face2ID}_${mouth2ID}.png`
      );
    }
  }
});
